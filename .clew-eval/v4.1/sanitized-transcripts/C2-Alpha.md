...
1. Search for `stripe` (case-insensitive) in all Python files, excluding `venv/`, `__pycache__/`, and `.worktrees/`
2. Search for Stripe-related patterns (`import stripe`, `stripe.api_key`, `stripe.Customer`, etc.)
...
5. Search across frontend source files (`*.ts`, `*.tsx`, `*.js`, `*.jsx`)
