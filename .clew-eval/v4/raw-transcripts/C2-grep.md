...
Searched for `stripe\.` across the entire repo. No matches found.
...
Searched for `import stripe|from stripe|stripe\.api|stripe\.PaymentIntent|stripe\.Customer|stripe\.Subscription|stripe\.Charge|stripe\.Invoice|stripe\.checkout|stripe\.webhook|stripe\.Price|stripe\.Product|StripeClient|stripe_sdk` across the backend. No matches.

## Search 6: Grep for payment-related keywords

Searched for `payment_intent|PaymentIntent|payment_method|checkout.session|webhook|STRIPE` across the backend. Found 71 files — these are all related to Shopify, Recharge, and other non-Stripe payment systems. No Stripe SDK usage found.
