...
## Search 1: search for "checkout source type"
Searched with the search tool for "checkout source type". Found highly relevant results:
- `backend/providers/admin.py` — `provider_checkout_source` method
- `backend/ecomm/utils.py` — `_determine_checkout_source` function with docstring explaining priority logic
- `backend/scripts/backfill_order_checkout_sources.py` — `_determine_checkout_source_from_payload`
- `backend/api/v1/serializers/consult.py` — `get_is_checkout_source_health_context`
- `backend/ecomm/models/order.py` — `Order` class

## Search 2: search for "checkout source enum"
Found:
- `backend/analytics/braze/constants.py` — `TestOrderSource` enum — but this is for Braze analytics, different concept
- `backend/ecomm/utils.py` — `_determine_checkout_source` again
- `backend/scripts/backfill_order_checkout_sources.py` — backfill logic

## Search 3: search for "CHECKOUT_SOURCE constants"
