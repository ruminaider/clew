# D1 Celery Task Enqueue Inventory - Clew Search Transcript
...
1. **Check available search modes**
   - Command: `clew search --help`
   - Purpose: Understand available search modes including `--exhaustive`

2. **Initial exhaustive searches for all three patterns**
   - `clew search ".delay(" --exhaustive --limit 20`
   - `clew search ".apply_async(" --exhaustive --limit 20`
   - `clew search "send_task(" --exhaustive --limit 20`
...
4. **Keyword searches for specific task types**
   - `clew search "submit_lab_order" --mode keyword --limit 15`
   - `clew search "shared_task" --mode keyword --limit 20 --language python`

5. **Grep fallback for completeness**
   - `grep -r "\.delay(" /Users/albertgwo/Work/evvy/backend --include="*.py" | grep -v test`
   - `grep -r "\.apply_async(" /Users/albertgwo/Work/evvy/backend --include="*.py" | grep -v test`
...
5. **Scheduled/Polling tasks** (from grep results, likely in celery beat config):
...
**Clew's exhaustive mode was highly effective** for this enumeration task:
- Found all three patterns in initial searches
- Grep results surfaced code snippets that confirmed locations
- Combined approach (clew + grep + Read) provided complete coverage
...
- Grep was useful for verification and finding exact line numbers
...
2. Grepping for all `.delay(` and `.apply_async(` calls
