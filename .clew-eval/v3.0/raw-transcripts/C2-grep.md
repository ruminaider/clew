...
## Task
Find all Stripe API calls in the /Users/albertgwo/Work/evvy codebase. List each call site with the Stripe API method being called and the context in which it's used.

## Search Process

### Step 1: Initial Broad Search for "stripe"
**Command:** `Grep pattern="stripe" path="/Users/albertgwo/Work/evvy" output_mode="files_with_matches" -i=true`
...
**Commands:**
- `Grep pattern="import stripe"`
- `Grep pattern="from stripe"`
...
**Command:** `Grep pattern="\.Customer\.|\.PaymentIntent\.|\.Charge\.|\.Subscription\.|\.Invoice\.|\.Refund\.|\.PaymentMethod\."`

**Result:** No matches found.

### Step 5: Search for Stripe Environment Variables
**Command:** `Grep pattern="STRIPE_SECRET|STRIPE_PUBLIC|STRIPE_API"`
...
**Command:** `grep -r "payment\|billing\|charge\|refund" /Users/albertgwo/Work/evvy/backend --include="*.py" | grep -i "def\|class"`
...
**Command:** `Grep pattern="stripe" -i=true` (without path restriction initially picked up documentation)

**Result:** References to "stripe" found only in clew project documentation files (not part of evvy codebase):
