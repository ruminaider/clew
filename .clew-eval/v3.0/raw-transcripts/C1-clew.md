# C1 - Clew Transcript: Find All Django URL Patterns Involving Ecomm-Related Views
...
- `clew search "ecomm urls django"`
- `clew search "url patterns ecommerce"`
- `clew search "urlpatterns path ecomm"`
...
`clew search "checkout cart order"` to find cart/checkout/order-related code.
...
`clew search "CartViewSet OrdersView"` to find the viewset implementations.
...
`clew search "ProductViewSet MalePartnerCheckoutView"` to find these views.
...
### Search 8: Webhooks
`clew search "shopify webhook recharge"` to find webhook views.
...
`clew search "PDPConfigurationViewSet subscription"` to find PDP and subscription views.
...
`clew search "get_subscription portal_link swap_subscription"` to find subscription view functions.
