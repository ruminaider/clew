# B1: Treatment Refill Function Impact Analysis - Grep Approach
...
**Tool**: Grep for function definition pattern
```bash
grep -i "def.*process.*shopify.*order" --output=content
```
...
**Tool**: Grep for function name usage
```bash
grep "process_shopify_order" --output=files_with_matches
```
...
**Tool**: Grep for actual function calls
```bash
grep "process_shopify_order_for_treatment_refill\(" --output=content
```
...
**Tool**: Grep for test methods
```bash
grep "test.*treatment.*refill|test.*replacement.*treatment" -i
```
...
**Tool**: Grep for helper function definitions
