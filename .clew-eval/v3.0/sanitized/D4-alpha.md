# D4 Evaluation Transcript

**Task:** Find all API endpoints that require authentication and identify which authentication method each uses (token auth, session auth, OAuth, API key, permission classes, etc.). Include decorator-based auth, middleware-based auth, and custom auth backends. Also identify any endpoints that are explicitly exempted from authentication.
