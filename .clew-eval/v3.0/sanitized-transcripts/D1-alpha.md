# D1 Celery Task Enqueue Inventory - Search Transcript
...
1. **Check available search modes**
   - Command: `search_help()`
   - Purpose: Understand available search modes

2. **Initial searches for all three patterns**
...
5. **Fallback search for completeness**
   - `search("\.delay(")`
   - `search("\.apply_async(")`
...
The search approach was highly effective for this enumeration task:
- Found all three patterns in initial searches
- Results surfaced code snippets that confirmed locations
